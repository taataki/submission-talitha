...
@st.cache_data
def load_data():
    url = "https://raw.githubusercontent.com/taataki/submission-talitha/a1615b5788748aab4fac321fdb5f8010f701bbbd/Dashboard/main_data.csv"
    df = pd.read_csv(url)
    df["dteday"] = pd.to_datetime(df["dteday"])

    # Load hour data for Q1 analysis
    ...
    for d in [df, df_hour]:
        if "season_label" not in d.columns:
            d["season_label"] = d["season"].map(season_map)
        if "weather_label" not in d.columns:
            d["weather_label"] = d["weathersit"].map(weather_map)
